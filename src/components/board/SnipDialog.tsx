...
export function SnipDialog({ onMakeCard }: { onMakeCard: (draft: CardDraft) => void }) {
  ...
  const copy = async () => {
    ...
    try {
      ...
      downloadBlob(snip.blob, `${snip.docName} стр.${snip.page}.png`);
    }
  };

  return (
    <Modal open={!!snip || busy} onClose={close} title="Изрезка" width={470}>
      {busy && !snip ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex shrink-0 gap-1.5">
              ...
              <button
                className="btn"
                onClick={() => downloadBlob(snip.blob, `${snip.docName} стр.${snip.page}.png`)}
              >
          ...
          <div>
            <div className="mb-1.5 label">Изпрати на дъска</div>
              ...
              {boards.map((b) => (
                ...
                >
                  ...
                  <span className="shrink-0 text-[11px] text-faint">
                    {b.board?.flow === 'scroll' ? 'свитък' : `${b.pageCount} стр.`}
                  </span>
                  {working === b.id && <Icon name="refresh" size={13} className="animate-spin" />}
                </button>
              ))}
              {boards.length === 0 && (
                <p className="px-2 py-2 text-[12px] text-faint">Още нямаш дъски — създай първата.</p>
              )}
            </div>
            <button className="btn mt-1.5 w-full" disabled={!!working} onClick={() => void createBoard()}>
              <Icon name="plus" size={14} />
              Нова дъска с тази изрезка
            </button>
          </div>
