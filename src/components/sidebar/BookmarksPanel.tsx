  ...
  return (
    <div className="scroll-thin h-full overflow-y-auto px-3 py-2 text-[12px]">
      <div className="mb-2 flex items-center justify-between">
        <span className="label">{t(L("Отметки", "Bookmarks"))}</span>
        <button className="btn h-7 px-2 text-[12px]" onClick={() => void toggleBookmark(currentPage)}>
          <Icon name="bookmark" size={13} />
          {t(bookmarks.some((b) => b.page === currentPage) ? L('Премахни', 'Remove') : L('Добави', 'Add'))}
        </button>
      </div>

      {bookmarks.length === 0 && <p className="mb-4 text-faint">{t(L("Няма отметки. Ctrl+D добавя текущата страница.", "No bookmarks. Ctrl+D adds the current page."))}</p>}
      ...
      <div className="mb-2 flex items-center justify-between">
        <span className="label">{t(L("Задачи", "Problems"))}</span>
        <span className="text-[11px] text-faint">{regions.length}</span>
      </div>

      {regions.length === 0 ? (
        <p className="text-faint">
          {t(L("С инструмента «Маркиране на задача» очертай задача и ѝ дай статус (двоен клик сменя статуса).", "With the \"mark a problem\" tool, frame a problem and give it a status (double-click cycles it)."))}
        </p>
      ) : (
        <>
            ...
            {regions.map((r) => (
              <li key={r.id}>
                ...
                >
                  ...
                  />
                  <span className="flex-1 truncate">{r.label || t(STATUS_LABEL[r.status])}</span>
